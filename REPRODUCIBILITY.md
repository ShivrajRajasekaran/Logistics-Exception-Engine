...
| Epochs | 40 | ~2m20s per epoch measured; `patience=15` stops early if val plateaus |
...
| Workers | 4 | Windows spawns workers as processes, so 8 costs more than it returns |
...
suggested ~2.2 s/iteration and a 27-hour run. The steady state is 3.6-3.7 it/s,
about 2m20s per epoch. Do not size a training budget from the first thirty
...
python scripts/train.py \
  ...
  --workers 4 \
  --device 0 \
  --name rtdetr_logistics_v1
```

Expected: ~2m20s per epoch, roughly 2 hours for 40 epochs. Best weights land at
`runs/train/rtdetr_logistics_v1/weights/best.pt`; copy that to `weights/best.pt`
...
| Epochs completed | 40 (no early stop) |
| Wall-clock training time | 1.981 hours |
| Steady-state throughput | 3.58-3.70 it/s, ~2m20s per epoch |
| Peak VRAM | 3.83 GB of 7.96 GB |
| Iterations per epoch | 502 |
...
| mAP@50 (all) | 0.601 |
| mAP@50-95 (all) | 0.527 |
| mAP@50 `package` | 0.994 |
| mAP@50 `damaged-package` | 0.208 |
